...
const limitAppointments = (appointments, limitPerDay = 2) => {
  ...
  const limitedAppointments = Object.values(groupedAppointments).flatMap(
    (dayAppointments) => {
...
const Events = () => {
  ...
  const { data: allTasks, isFetching } = useQuery({
    ...
    initialData: [],
    queryFn: () =>
      axiosSecure.get(`/userTasksAllEvents/${user?.uid}`).then((res) => {
        return res.data;
      }),
  });
  const { data: tasksAmount, isFetching2 } = useQuery({
    ...
    initialData: {},
    queryFn: () =>
      axiosSecure.get(`/userTasksAllAmounts/${user?.uid}`).then((res) => {
        return res.data;
      }),
  });
    ...
    <div className="w-10/12 mx-auto ">
      ...
      <div className="flex gap-3 md:gap-10 flex-col md:flex-row justify-between md:justify-center py-2 w-11/12 mx-auto mt-4">
        <h1 className="font-bold text-blue-700">
          Upcoming Tasks: {tasksAmount.upcomingTasksLength}
        </h1>
        <h1 className="font-bold text-green-300">
          Completed Tasks: {tasksAmount.finishedTasksLength}
        </h1>
        <h1 className="font-bold text-red-500">
          Unfinished Tasks: {tasksAmount.unfinishedTasksLength}
        </h1>
      </div>
      <motion.div
        initial={{ scale: 0, x: 500, opacity: 0 }}
        animate={{ scale: 1, x: 0, opacity: 1 }}
        transition={{ duration: 1 }}
        className="w-full md:w-11/12 mx-auto mt-3"
      >
        <Paper className="w-full ">
          <Scheduler data={newAppointments}>
            <ViewState />
            <MonthView timeTableCellComponent={DayCellComponent} />
            <Toolbar />
            <DateNavigator />
            <TodayButton />
...
const CustomAppointment = ({ children, ...restProps }) => {
  ...
  if (status === "finished") {
    ...
    className = `bg-red-500 text-white font-semibold text-sm relative ${
      isMore ? "bg-gray-300 text-gray-700 text-xs" : ""
    }`;
  }
          ...
          <div className="flex flex-col gap-2 border rounded-xl p-3  bg-gradient-to-r from-indigo-400 to-cyan-400 border-none w-full">
            <div className="flex w-full justify-between items-center bg-transparent">
              <label className="px-0 input outline-0 border-0 flex items-center gap-2 border-none outline-none focus-within:outline-none h-fit bg-transparent">
                <p
                  ...
                  ref={nameRef}
                  className={`outline-none w-full max-w-[250px] cursor-text text-black font-bold  focus-within:before:content-none bg-transparent ${
                    taskName ? "" : "before:content-['Task_name'] text-gray-600"
                  }`}
            ...
            <label className="px-0 input  flex items-center gap-2  border-none outline-none focus-within:outline-none h-fit bg-transparent">
              <p
                ...
                suppressContentEditableWarning={true}
                ref={descriptionRef}
                className={`outline-none w-full cursor-text  focus-within:before:content-none bg-transparent${
                  descripiton
                    ? "text-black"
                    : "before:content-['Description'] text-gray-300"
                }`}
            ...
            <div className="flex items-center gap-3">
              <div className="dropdown ">
                <div
                  tabIndex={0}
                  role="button"
                  className="btn m-1  bg-gradient-to-r from-indigo-400 to-cyan-400 text-black border-none text-white"
                >
                  Due Date
                </div>
                <ul
                  tabIndex={0}
                  className="dropdown-content menu b bg-gradient-to-r from-indigo-400 to-cyan-400  rounded-box z-30 w-52 p-2 shadow text-white"
                >
                  <div>
                    <label>Date</label> <br />
                  ...
                  <div className="mt-2">
                    ...
                    <input
                      type="time"
                      ref={timeRef}
                      className="w-full text-black border p-2 outline-none focus-within:outline-none"
                    />
                  </div>
                </ul>
              </div>
              ...
              <div className="dropdown ">
                <div
                  tabIndex={0}
                  role="button"
                  className="btn m-1  bg-gradient-to-r from-indigo-400 to-cyan-400  text-white border-none "
                >
                  Reminder
                </div>
                <ul
                  tabIndex={0}
                  className="dropdown-content menu bg-base-100 rounded-box z-[30] w-52 p-2 shadow right-0"
                >
                  <div className="mt-2">
                    <label>Time</label> <br />
                    <input
                      type="time"
                      ref={reminderRef}
                      className="w-full border p-2 outline-none focus-within:outline-none"
                    />
                  </div>
                </ul>
              </div>
              <Select ref={priorityRef} options={options} className="text-sm" />
            </div>
            <form method="dialog" className="flex w-full justify-end gap-3">
              <button
                className="btn  bg-gradient-to-r from-indigo-400 to-cyan-400  text-white border-none"
                onClick={addTask}
              >
                Add Task
              </button>
